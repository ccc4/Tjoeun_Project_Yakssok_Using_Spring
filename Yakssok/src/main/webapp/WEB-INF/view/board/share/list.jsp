				...
				<div class="_board_list_table">
					<table class="table table-hover" style="table-layout:fixed; word-break:break-all;">
						...
						<tbody>
							...
							<tr>
								<td>${l.b_idx }</td>
								<td class="_board_list_title"><a href="${pageContext.request.contextPath}/board/share/view/${l.b_idx}">${l.title }</a></td>
								<td>${l.nickname }</td>
					...
					<c:if test="${!empty loginMember }">
						<input type="button" class="btn btn-default btn-sm pull-right" value="글작성" onclick="location.href='${pageContext.request.contextPath }/board/share/write'">
					</c:if>
				</div>
				<!-- 게시판 목록 끝 -->
						...
						<ul class="pagination pagination-sm">
							<c:if test="${p.page != 1 }">
								<c:if test="${empty p.option }">
									<li><a href="${pageContext.request.contextPath }/board/share">처음</a></li>
								</c:if>
								<c:if test="${!empty p.option }">
									<li><a href="${pageContext.request.contextPath }/board/share?option=${p.option}&keyword=${p.keyword}">처음</a></li>
								</c:if>
							</c:if>
							<c:if test="${p.page == 1 }">
							...
							<c:if test="${p.startPage != 1 }">
								<c:if test="${empty p.option }">
									<li><a href="${pageContext.request.contextPath }/board/share?page=${p.startPage - 1}">이전</a></li>
								</c:if>
								<c:if test="${!empty p.option }">
									<li><a href="${pageContext.request.contextPath }/board/share?page=${p.startPage - 1}&option=${p.option}&keyword=${p.keyword}">이전</a></li>
								</c:if>
							</c:if>
							<c:if test="${p.startPage == 1 }">
							...
							<c:forEach var="i" begin="${p.startPage }" end="${p.endPage }">
								...
								<c:if test="${i != p.page }">
									<c:if test="${empty p.option }">
										<li><a href="${pageContext.request.contextPath }/board/share?page=${i}">${i }</a></li>
									</c:if>
									<c:if test="${!empty p.option }">
										<li><a href="${pageContext.request.contextPath }/board/share?page=${i}&option=${p.option}&keyword=${p.keyword}">${i }</a></li>
									</c:if>
								</c:if>
							</c:forEach>
							...
							<c:if test="${p.endPage != p.totalPage }">
								<c:if test="${empty p.option }">
									<li><a href="${pageContext.request.contextPath }/board/share?page=${p.endPage + 1}">다음</a></li>
								</c:if>
								<c:if test="${!empty p.option }">
									<li><a href="${pageContext.request.contextPath }/board/share?page=${p.endPage + 1}&option=${p.option}&keyword=${p.keyword}">다음</a></li>
								</c:if>
							</c:if>
							<c:if test="${p.endPage == p.totalPage }">
							...
							<c:if test="${p.page != p.totalPage }">
								<c:if test="${empty p.option }">
									<li><a href="${pageContext.request.contextPath }/board/share?page=${p.totalPage}">끝</a></li>
								</c:if>
								<c:if test="${!empty p.option }">
									<li><a href="${pageContext.request.contextPath }/board/share?page=${p.totalPage}&option=${p.option}&keyword=${p.keyword}">끝</a></li>
								</c:if>
							</c:if>
							<c:if test="${p.page == p.totalPage }">
